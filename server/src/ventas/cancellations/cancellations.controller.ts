import { Body, ConflictException, Controller, Get, NotFoundException,  Param, Post, Delete, Put, HttpCode } from '@nestjs/common';
...
export class CancellationsController {
    ...
    async findOne(@Param("id") id : string){
        try {
            ...
            if(!selectedCancellation){
                throw new NotFoundException("No se encontro la cancelation de esta venta")
            }
    ...
    async delete(@Param("id") id : string){
        try {
            ...
            if(!deletedCancellation){
                throw new NotFoundException("No se puso eliminar esta nota por que no existe")
            }
    ...
    async update(@Param("id") id : string, @Body() body : UpdateCancellationDto){
        try {
            const updatedCancellation = await this.cancellationService.update(id, body);
