...
export class CategoriesController {
    ...
    async findAll() {
        try {
            ...
            if (!categoriesArray || categoriesArray.length === 0) throw new NotFoundException('No se encontraron categorías.'); 
            return categoriesArray;
        } catch (error) {
            throw new ConflictException('Ocurrió un error inesperado al buscar categorías.');
        }
    }
@Get(":id")
...
async delete(@Param("id") id: string){
    try {
        ...
        if ( !categoryDeleted ) throw new NotFoundException("La categoria no existe")
...
async update(@Param("id") id: string, @Body() body: UpdateCategoryDto){
    try {
        ...
        if ( !categoryUpdated ) throw new NotFoundException("No se encontro la categoria")
